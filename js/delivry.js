$(document).ready(function () {
	// Armazena a posição inicial do menuCategorias
	let menuCategoriasOffset = 0;
	if ($('#menuCategorias').length > 0) {
		menuCategoriasOffset = $('#menuCategorias').offset().top;
	}

	$(window).scroll(function () {
		if ($('#menuCategorias').length > 0) {
			let windowTop = $(window).scrollTop();
			$('#menuCategorias').toggleClass('fixed', windowTop > menuCategoriasOffset);
		}
	});
